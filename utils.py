...
def judge_inside(vertices, p):
    ...
    # return -(C1 XOR C2) and -(C2 XOR C3)
...
def x_jagged(array, count_num, start_point , interval):
    max_range = 32
    final_array = array

    for x_value in range(start_point, max_range, interval):
        final_array = np.reshape(final_array,(-1,2))
        if np.any(final_array[:, 0] == x_value):
            if len(final_array[final_array[:, 0] == x_value]) < 1:
                continue
    
            temp_array = final_array[final_array[:, 0] == x_value]
            if count_num == 0:#right
                new_element1 = np.clip(np.array([temp_array[0, 0], temp_array[0, 1] - 1]),0,31)
                new_element2 = np.clip(np.array([temp_array[0, 0], temp_array[0, 1] - 2]),0,31)
                new_element3 = np.clip(np.array([temp_array[0, 0], temp_array[0, 1] - 3]),0,31)
                new_element4 = np.clip(np.array([temp_array[0, 0], temp_array[0, 1] - 4]),0,31)
            elif count_num == 2:#left
                new_element1 = np.clip(np.array([temp_array[0, 0], temp_array[-1, 1] + 1]),0,31)
                new_element2 = np.clip(np.array([temp_array[0, 0], temp_array[-1, 1] + 2]),0,31)
                new_element3 = np.clip(np.array([temp_array[0, 0], temp_array[-1, 1] + 3]),0,31)
                new_element4 = np.clip(np.array([temp_array[0, 0], temp_array[-1, 1] + 4]),0,31)

            # 將修改後的陣列回寫到原始陣列中
            final_array = np.append(final_array,new_element1)
            if count_num == 0:
                final_array = np.append(final_array,new_element2)
            # final_array = np.append(final_array,new_element3)
            # final_array = np.append(final_array,new_element4)
    
    return final_array

def y_jagged(array, count_num, start_point , interval):
    max_range = 32
    final_array = array

    for y_value in range(start_point, max_range, interval):
        final_array = np.reshape(final_array,(-1,2))
        if np.any(final_array[:, 1] == y_value):
            if len(final_array[final_array[:, 1] == y_value]) < 1:
                continue
            temp_array = final_array[final_array[:, 1] == y_value]
            if count_num == 3:#up
                new_element1 = np.clip(np.array([temp_array[0, 0] - 1, temp_array[0, 1]]),0,31)
                new_element2 = np.clip(np.array([temp_array[-1, 0] + 1, temp_array[-1, 1]]),0,31)
                new_element3 = np.clip(np.array([temp_array[0, 0] - 2, temp_array[0, 1]]),0,31)
                new_element4 = np.clip(np.array([temp_array[-1, 0] + 2, temp_array[-1, 1]]),0,31)
                new_element5 = np.clip(np.array([temp_array[0, 0] - 3, temp_array[0, 1]]),0,31)
                new_element6 = np.clip(np.array([temp_array[-1, 0] + 3, temp_array[-1, 1]]),0,31)
                new_element7 = np.clip(np.array([temp_array[0, 0] - 4, temp_array[0, 1]]),0,31)
                new_element8 = np.clip(np.array([temp_array[-1, 0] + 4, temp_array[-1, 1]]),0,31)
            elif count_num == 1:#down
                new_element1 = np.clip(np.array([temp_array[0, 0] - 1, temp_array[-1, 1]]),0,31)
                new_element2 = np.clip(np.array([temp_array[-1, 0] + 1, temp_array[-1, 1]]),0,31)
                new_element3 = np.clip(np.array([temp_array[0, 0] - 2, temp_array[-1, 1]]),0,31)
                new_element4 = np.clip(np.array([temp_array[-1, 0] + 2, temp_array[-1, 1]]),0,31)
                new_element5 = np.clip(np.array([temp_array[0, 0] - 3, temp_array[-1, 1]]),0,31)
                new_element6 = np.clip(np.array([temp_array[-1, 0] + 3, temp_array[-1, 1]]),0,31)
                new_element7 = np.clip(np.array([temp_array[0, 0] - 4, temp_array[-1, 1]]),0,31)
                new_element8 = np.clip(np.array([temp_array[-1, 0] + 4, temp_array[-1, 1]]),0,31)
            ...
            # final_array = np.append(final_array,new_element3)
            # final_array = np.append(final_array,new_element4)
            # final_array = np.append(final_array,new_element5)
            # final_array = np.append(final_array,new_element6)
            # final_array = np.append(final_array,new_element7)
            # final_array = np.append(final_array,new_element8)

    return final_array

def jagged(x_list, y_list, count_num):
    if len(x_list) == 0:
    ...
    if count_num == 0:
        final_array = x_jagged(temp_array, count_num, 1, 4) #1, 4
    elif count_num == 1:
        final_array = y_jagged(temp_array, count_num, 0, 3) #1, 3/4
    elif count_num == 2: 
        final_array = x_jagged(temp_array, count_num, 1, 3) #1, 3
    elif count_num == 3:
        final_array = y_jagged(temp_array, count_num, 2, 4) #1 2

    final_array = np.reshape(final_array,(-1,2))
    final_x_list = final_array[:,0]
    final_y_list = final_array[:,1]
...
def draw_shadow(position, image, pos_list, coefficient, count_num):
    ...
    # final_x_list = x_list
    # final_y_list = y_list
    final_x_list, final_y_list = jagged(x_list, y_list, count_num)

    shadow_area[final_x_list, final_y_list] = 255
    image[final_x_list, final_y_list, 0] = np.round(image[final_x_list, final_y_list, 0] * coefficient).astype(np.uint8) 
