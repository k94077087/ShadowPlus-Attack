...
class Particle:
    ...
    def __init__(
        ...
        array = np.random.uniform(coord_min, coord_max, dim - 4)
        if count_num == 0: #right
            ...
            append = [0, 0, 0, 31]
        array = np.append(array,append)
        self.pos = polygon_correction(array)
...
class PSO:
    ...
    Args:
        ...
        count_num:Determine the masking types.
    """
    def __init__(
        self,
        dim: int = 6,
        ...
        **parameters
        ...
        self.Particle_list = [
            Particle(dim, coord_min, coord_max, max_speed, count_num) for _ in range(size)]
    ...
    def fit_fun(self, position, **parameters):
        ...
        if self.physical:
            ...
            img, shadow_area = draw_shadow(position, self.image, self.coord, self.coefficient, self.count_num)
